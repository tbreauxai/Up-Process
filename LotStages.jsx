...
import { UserCircle, MessageSquare, Camera, Smartphone, Zap, Clock, PhoneForwarded } from 'lucide-react';
import { discoveryFields } from './data.js';
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 p-6 bg-blue-50/50 rounded-3xl border-2 border-blue-100">
            <div className="flex items-center space-x-4 bg-white p-5 rounded-2xl border-2 border-slate-100 shadow-sm">
              <input type="checkbox" id="hasLicense" name="hasLicense" checked={leadDetails.hasLicense} onChange={handleInputChange} className="w-8 h-8 text-blue-600 rounded-lg border-slate-300 focus:ring-blue-500" />
            ...
            <div className="bg-white p-5 rounded-2xl border-2 border-slate-100 shadow-sm">
              ...
              <input type="tel" name="phoneNumber" value={leadDetails.phoneNumber} onChange={handleInputChange} className="w-full p-2 text-xl font-bold border-b-2 border-slate-100 focus:border-blue-500 outline-none transition-all" placeholder="(555) 000-0000" />
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                {discoveryFields.map(field => (
                  !leadDetails[field.name] && (
                    <div key={field.name} className={field.fullWidth ? "sm:col-span-2" : ""}>
                      <label className="block text-sm font-bold text-slate-700 mb-1">{field.label}</label>
                      <input type="text" name={field.name} value={leadDetails[field.name]} onChange={handleInputChange} className="w-full p-4 text-base border-2 border-slate-100 rounded-2xl focus:border-blue-500 outline-none font-bold" placeholder={field.placeholder} />
                    </div>
                  )
                ))}
              </div>
