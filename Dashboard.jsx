...
import { Phone, Users, ClipboardList, Camera, CheckCircle2, Trash2, Sparkles, Copy, Loader2 } from 'lucide-react';
import { lotStages, phoneStages, phoneChecklist, lotChecklist } from '../constants/data';
...
export default function Dashboard({ 
  ...
  return (
    <>
      ...
      {hasData() && (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-6 duration-500 mt-6">
            ...
            <div key={lead.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-start space-x-4">
              ...
              <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-2 pt-4 mt-2 border-t border-slate-100">
                     <button 
                       onClick={() => generateGeminiContent(lead.id, 'crm', lead.rawData)}
                       disabled={generatingId === lead.id}
                       className="flex items-center px-3 py-2 bg-indigo-50 text-indigo-700 rounded-xl text-[10px] font-black uppercase tracking-wider hover:bg-indigo-100 transition-colors disabled:opacity-50"
                     >
                       {generatingId === lead.id && generatingType === 'crm' ? <Loader2 className="w-3 h-3 mr-1.5 animate-spin" /> : <Sparkles className="w-3 h-3 mr-1.5" />}
                       ✨ Gen CRM Note
                     </button>
                     <button 
                       onClick={() => generateGeminiContent(lead.id, 'sms', lead.rawData)}
                       disabled={generatingId === lead.id}
                       className="flex items-center px-3 py-2 bg-pink-50 text-pink-700 rounded-xl text-[10px] font-black uppercase tracking-wider hover:bg-pink-100 transition-colors disabled:opacity-50"
                     >
                  ...
                  {lead.crmNote && (
                    <div className="mt-4 p-4 bg-indigo-50/50 border border-indigo-100 rounded-2xl relative group">
                      ...
                      <button onClick={() => copyToClipboard(lead.crmNote)} className="absolute top-4 right-4 text-indigo-300 hover:text-indigo-600 transition-colors">
                  ...
                  {lead.followUpSms && (
                    <div className="mt-4 p-4 bg-pink-50/50 border border-pink-100 rounded-2xl relative group">
                      ...
                      <button onClick={() => copyToClipboard(lead.followUpSms)} className="absolute top-4 right-4 text-pink-300 hover:text-pink-600 transition-colors">
                        <Copy className="w-4 h-4" />
                      </button>
                    </div>
                  )}
